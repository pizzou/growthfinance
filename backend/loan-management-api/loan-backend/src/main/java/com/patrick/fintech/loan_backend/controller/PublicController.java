...
public class PublicController {
    ...
    public ResponseEntity<ApiResponse<String>> submitContact(
        ...
        List<User> staff = userRepo.findByOrganization(org)
            ...
            .filter(u ->
                u.getRole() != null &&
                Set.of("ADMIN", "MANAGER").contains(u.getRole().getName())
            )
            .toList();
    ...
    public ResponseEntity<BorrowerDashboardResponse> borrowerDashboard(
        ...
        return ResponseEntity.ok(
            loanService.getBorrowerDashboard(reference, phone)
        );
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> uploadApplicationDocument(
        ...
        if (borrower == null) {
            throw new RuntimeException("Borrower not found for this application.");
        }

        var saved = fileService.upload(
            borrower.getId(),
            file,
            docType,
            true
        ...
        return ResponseEntity.ok(
            ApiResponse.ok("Document uploaded", result)
        );
    }
    ...
    listApplicationDocuments(
        ...
        List<Map<String, Object>> docs =
            fileService.getByBorrowerMetadataOnly(borrower.getId())
                .stream()
                .map(f -> {
    ...
    public ResponseEntity<ApiResponse<Void>> deleteApplicationDocument(
        ...
        if (file.getBorrower() == null ||
            loan.getBorrower() == null ||
            !file.getBorrower().getId().equals(loan.getBorrower().getId())) {
        ...
        if (!file.isUploadedByApplicant()) {
            throw new RuntimeException(
                ...
                "contact us if it needs changing."
            );
        }
        ...
        if (file.getVerificationStatus() == VerificationStatus.VERIFIED) {
            throw new RuntimeException(
                ...
                "Contact us if it needs to be replaced."
            );
        }
        ...
        auditService.log(
            ...
            fileId.toString(),
            documentType +
                " (" + fileName + ") removed by applicant for application " +
                loan.getReferenceNumber()
        );

        return ResponseEntity.ok(
            ApiResponse.ok("Document removed", null)
        );
    }
    ...
    getApplicationComments(
        ...
        List<Map<String, Object>> comments =
            loanCommentRepo.findVisibleToApplicantByLoanId(loan.getId())
                .stream()
                .map(c -> {
                    Map<String, Object> m = new LinkedHashMap<>();
    ...
    initiatePublicPayment(
        ...
        BigDecimal requestedAmount = num(body.get("amount"));
        ...
        com.patrick.fintech.loan_backend.dto.PaymentGatewayRequest req =
            new com.patrick.fintech.loan_backend.dto.PaymentGatewayRequest();

        /*
         * Flutterwave request DTO may still use Double.
         * Keep the application's financial model as BigDecimal and
         * convert only at the external gateway boundary.
         */
        req.setAmount(dueAmount.doubleValue());
        ...
        com.patrick.fintech.loan_backend.dto.PaymentGatewayResponse gw =
        ...
        result.put("redirectUrl", gw.getRedirectUrl());

        if ("success".equalsIgnoreCase(gw.getStatus())) {
            ...
            paymentService.recordPayment(
                ...
                gw.getTransactionId(),
                "GATEWAY",
                "Paid via Flutterwave (borrower self-service)",
                null
            );
            ...
            return ResponseEntity.ok(
                ApiResponse.ok("Payment completed", result)
            );
        }

        if ("pending".equalsIgnoreCase(gw.getStatus())) {
            ...
            auditService.log(
                ...
                "Borrower self-service payment of " +
                    ...
                    " " +
                    loan.getCurrency() +
                    " initiated (pending confirmation) for loan " +
                    loan.getReferenceNumber()
            );
    ...
    public ResponseEntity<byte[]> downloadAgreement(
        ...
        return pdfResponse(
            pdf,
            "Loan-Agreement-" + loan.getReferenceNumber()
        );
    }
    ...
    public ResponseEntity<byte[]> downloadSchedule(
        ...
        return pdfResponse(
            pdf,
            "Repayment-Schedule-" + loan.getReferenceNumber()
        );
    }
    ...
    public ResponseEntity<byte[]> downloadReceipt(
        ...
        return pdfResponse(
            pdf,
            "Disbursement-Receipt-" + loan.getReferenceNumber()
        );
    }
    ...
    private List<Map<String, Object>> agreementRows(Loan loan) {
        ...
        add.accept(
            "Interest Rate",
            loan.getInterestRate() + "% per month"
        );

        add.accept(
            "Duration",
            loan.getDurationMonths() + " months"
        );
        ...
        add.accept(
            "Total Repayable",
            loan.getCurrency() + " " + loan.getTotalRepayable()
        );

        add.accept(
            "Processing Fee",
            loan.getCurrency() + " " + loan.getProcessingFee()
        );
    ...
    private List<Map<String, Object>> receiptRows(Loan loan) {
        ...
        add.accept(
            "Amount Disbursed",
            loan.getCurrency() + " " + loan.getDisbursedAmount()
        );
    ...
    private Loan verifyOwnership(
        ...
        if (reference == null || phone == null ||
            reference.isBlank() || phone.isBlank()) {

            throw new RuntimeException(
                "Reference number and phone number are required."
            );
        }

        Loan loan =
            loanRepo.findByReferenceNumber(
        ...
        if (borrower == null ||
            ...
            !borrower.getPhoneHash().equals(suppliedHash)) {
    ...
    private List<Map<String, Object>> progressSteps(
        ...
        if (loan.getBorrower() != null) {
            ...
            docsUploaded =
                Boolean.TRUE.equals(
                    docReq.get("readyToApprove")
                );

            docsVerified =
                Boolean.TRUE.equals(
                    docReq.get("readyToDisburse")
                );
        }
    ...
    private List<Map<String, Object>> timeline(
        ...
        List<Map.Entry<LocalDateTime, String>> raw =
            new ArrayList<>();
        ...
        if (loan.getBorrower() != null) {
            ...
            List<BorrowerFile> files =
                fileService.getByBorrowerMetadataOnly(
                    loan.getBorrower().getId()
                );

            files.stream()
                .map(BorrowerFile::getUploadedAt)
            ...
            files.stream()
                .map(BorrowerFile::getVerifiedAt)
                .filter(Objects::nonNull)
        ...
        if (loan.getApprovedAt() != null) {
            raw.add(
                Map.entry(
                    loan.getApprovedAt().atStartOfDay(),
                    "Loan approved"
                )
            );
        }
        ...
        if (loan.getDisbursedAt() != null) {
            raw.add(
                Map.entry(
                    loan.getDisbursedAt().atStartOfDay(),
                    "Loan disbursed"
                )
            );
        }
    ...
    getTenantConfig(
        ...
        config.put(
            ...
            parseListOrDefault(
                ...
                defaultTestimonials(org.getName())
            )
        );
    ...
    submitApplication(
        ...
        BigDecimal amount = num(body.get("amount"));
        ...
        boolean acceptedTerms =
            ...
            Boolean.parseBoolean(
                body.get("acceptedTerms").toString()
            );
        ...
        borrower.setSingleCertificateNumber(
            str(body.get("singleCertificateNumber"))
        );

        borrower.setSpouseFullName(
            str(body.get("spouseFullName"))
        );

        borrower.setSpouseNationalId(
            str(body.get("spouseNationalId"))
        );

        borrower.setSpousePhone(
            str(body.get("spousePhone"))
        );

        borrower.setSpouseConsent(
            body.get("spouseConsent") != null
                ? Boolean.parseBoolean(
                    body.get("spouseConsent").toString()
                )
                : null
        );
        ...
        borrower.setEmploymentType(
            str(body.get("employmentType"))
        );

        borrower.setEmployerName(
            str(body.get("employerName"))
        );

        borrower.setJobTitle(
            str(body.get("jobTitle"))
        );

        borrower.setMonthlyIncome(
            num(body.get("monthlyIncome"))
        );

        borrower.setMonthlyExpenses(
            num(body.get("monthlyExpenses"))
        );
        ...
        LoanRequest req =
            LoanRequest.builder()
                ...
                .collateralValue(
                    num(body.get("collateralValue"))
                )
                .collateralDescription(
                    str(body.get("collateral"))
                )
                .loanType(
                    mapLoanType(
        ...
        try {
            ...
            log.info(
                "[EMAIL SUCCESS] Dispatched application received alert to: {}",
                borrower.getEmail()
            );
            ...
            log.error(
                "[EMAIL CRITICAL FAILURE] Outbound loan receipt transmission failed",
                e
            );
        }
    ...
    private List<Map<String, Object>> parseListOrDefault(
        ...
        try {
            ...
            log.warn(
                "Could not parse stored CMS JSON, using default: {}",
                e.getMessage()
            );
    ...
    private List<Map<String, Object>> defaultStats() {
        ...
        String[][] rows = {
            {"👥", "5,000+", "Happy Clients"},
            {"💰", "RWF 2B+", "Loans Disbursed"},
            {"⚡", "24 hrs", "Average Approval"},
    ...
    private List<Map<String, Object>> defaultTestimonials(
        ...
        t.add(
            Map.of(
                ...
                "The process was fast and the staff were very professional."
            )
        );
        ...
        t.add(
            Map.of(
                ...
                "rating", 5,
                "text",
                "I got an agricultural loan to buy quality seeds and farming equipment. " +
                "My harvest doubled this season. Thank you " +
                orgName +
                "!"
            )
        );
        ...
        t.add(
            Map.of(
                ...
                "rating", 5,
                "text",
                "The salary advance helped me cover my children's school fees on time. " +
                "Easy application, same-day approval. Excellent service!"
            )
        );
    ...
    private List<Map<String, Object>> buildProducts() {
        ...
        String[][] defaults = {
            {
                ...
                "5000000",
                "36",
                "Fast personal financing for any purpose"
            },
            {
                "Business Loan",
                "🏢",
            ...
            },
            {
                ...
                "10000000",
                "24",
                "Seasonal farming and agribusiness finance"
            },
            {
                "SME Finance",
                "📦",
                ...
                "30000000",
                "48",
                "Tailored finance for small businesses"
            },
            {
                "Salary Advance",
                "💵",
                ...
                "2000000",
                "3",
                "Quick advance on your monthly salary"
            },
            {
                "Microfinance",
                "💡",
                ...
                "500000",
                "12",
                "Small loans for micro-entrepreneurs"
            }
        };
        ...
        for (String[] p : defaults) {
            ...
            m.put("title", p[0]);
            m.put("icon", p[1]);
            m.put("rate", p[2] + "% / month");
            m.put("maxAmount", p[3]);
            m.put("term", "up to " + p[4] + " months");
            m.put("description", p[5]);
    ...
    private Organization resolveOrg(String slug) {
        ...
        String normalizedSlug =
            slug.toLowerCase().replace("-", "").replace(" ", "");
    ...
    /**
     * All internal monetary values use BigDecimal.
     * This prevents Double precision problems for financial data.
     */
