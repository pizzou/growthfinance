 ...
 * IMPORTANT
 *
 * The tracking endpoint provides StatusResult.
 * Dashboard information is optional because the dashboard
 * endpoint can fail independently.
 */
...
/**
 * Merge dashboard data into an EXISTING TrackResult.
 *
 * DashboardResult alone is NOT a TrackResult because it does
 * not contain required StatusResult fields.
 */
const mergeDashboardResult = (
  previous: TrackResult | null,
...
/**
 * Merge fresh application status into an EXISTING TrackResult.
 */
...
export default function TrackPage() {
  ...
  const [result, setResult] =
    useState<TrackResult | null>(null);
  ...
  const [showPaySheet, setShowPaySheet] =
    useState(false);

  const [payChoice, setPayChoice] =
    useState(0);
  ...
  const [payMessage, setPayMessage] =
    useState('');
  ...
  const handleSubmit = async (
    ...
    if (!ph) {
      setError(
        'Please enter the phone number used during your application.'
      );
    ...
    try {
      ...
      /*
       * StatusResult is the foundation of TrackResult.
       */
      ...
      try {
        ...
        console.error(
          '[TRACK] Dashboard request failed',
          {
            status:
              dashboardErr?.response?.status,

            statusText:
              dashboardErr?.response?.statusText,

            response:
              dashboardErr?.response?.data,

            message:
              dashboardErr?.message,

            request:
              dashboardErr?.config?.data,

            url:
              dashboardErr?.config?.url,

            method:
              dashboardErr?.config?.method,
          }
        );
        ...
        if (
          ...
          console.warn(
            '[TRACK] Dashboard returned 400. ' +
            'Application status was successfully retrieved.'
          );
        }
      }
      ...
      publicApi
        ...
        .catch(
          (documentError: unknown) => {
            console.error(
              '[TRACK] Documents request failed:',
              documentError
            );
      ...
      console.error(
        '[TRACK] Application tracking failed:',
        {
          status:
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!paymentReference) {
        setError(
          'Unable to determine the loan reference.'
        );
      ...
      if (!paymentPhone) {
        setError(
          'Unable to determine the phone number.'
        );
      ...
      if (data?.recorded) {
         ...
         * as the null branch.
         *
         * DashboardResult is not TrackResult.
         */
        ...
        try {
          ...
          console.error(
            '[PAYMENT] Dashboard refresh failed:',
            refreshError
          );
        }
      } else {
  ...
  const handleDownloadDoc = async (
    ...
    try {
      ...
      if (!ref || !ph) {
        toast(
          'error',
          'Missing application reference or phone number.'
        );
    ...
    try {
      ...
      const docs =
        Array.isArray(
          ...
          ? docsResponse as UploadedDoc[]
          : [];

      const status =
        statusResponse as StatusResult;
       ...
       * It can be merged into the existing TrackResult.
       *
       * It must NOT be returned directly when prev is null.
       */
      setResult(prev =>
        mergeStatusResult(
          prev,
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-14 md:py-20">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 text-white/75 text-sm md:text-base max-w-xl leading-7">
              ...
              monitor repayment, upload documents,
              download loan records and make payments
              from one place.
            </p>
            ...
            {isActiveLoan && (
              <>
                ...
                <div
                  ...
                  style={{
                    background:
                      'linear-gradient(135deg, #063B25 0%, #0D6B3E 100%)',
                  }}
                >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            ...
            <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden">
              ...
              <div
                ...
                style={{
                  background:
                    'linear-gradient(135deg, #063B25, #0D6B3E)',
                }}
              >
