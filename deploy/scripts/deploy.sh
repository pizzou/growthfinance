...
until docker-compose exec -T postgres pg_isready -U loansaas -d loansaas_growthfinance 2>/dev/null; do
    echo -n "."; sleep 2
done
echo ""
...
until docker-compose exec -T backend wget -qO- http://localhost:8080/actuator/health 2>/dev/null | grep -q '"status":"UP"'; do
